...
- **No implementation details in the README** — no format notes, parsing pipelines, measured
  tables or "how we figured it out" stories. Those go to `docs/internals.md`.
...
- Prefix with a scope, e.g. `feat(dedupe): …`, `fix(analysis): …`, `docs(repo): …`,
  `chore(repo): …`.
